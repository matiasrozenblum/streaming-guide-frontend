...
async function getInitialData(): Promise<InitialData> {
  try {
    ...
    // Use V2 optimized endpoints with batched Redis reads for the initial load
    const todayPromise = fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/channels/with-schedules/today/v2?live_status=true`,
      {
        next: { revalidate: 300 }
      }
    ).then(res => res.json());
    ...
    const [holidayData, todaySchedules, categories, categoriesEnabledData, streamersEnabledData, banners] = await Promise.all([
      holidayPromise,
      todayPromise,
      categoriesPromise,
      categoriesEnabledPromise, // Use correct variable name for the promise
    ...
    return {
      ...
      weekSchedules: Array.isArray(todaySchedules) ? todaySchedules : [], // Load V2 today as initial "week" data so grid renders instantly
