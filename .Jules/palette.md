...
## 2025-02-18 - Missing ARIA Labels on Dismiss Buttons
**Learning:** Discovered an accessibility issue in tooltips (like ZappingTooltip) where the icon-only 'close' button lacked an 'aria-label', making it inaccessible to screen reader users.
**Action:** Ensure all dismiss/close buttons, particularly in tooltips and overlays, contain explicit 'aria-label' attributes in Spanish (e.g., 'Cerrar') to clearly indicate their function.
