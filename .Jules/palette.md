...
## 2026-09-01 - Context-rich ARIA labels on multi-step forms
**Learning:** In multi-step flows like the authentication wizard, secondary navigation buttons like 'Volver' should include a descriptive `aria-label` (e.g., 'Volver al paso anterior') to provide better context for screen reader users, rather than relying solely on the visual label.
**Action:** Add descriptive `aria-label` attributes to secondary navigation buttons in wizard flows.
